...
import {
    ...
    setCurrentMassAction,
    startRecipeAction,
} from 'store/ActiveRecipe/slice';
...
const CURRENT_INGREDIENT_CHARACTERISTIC_UUID =
    '8751470e-3db3-42b9-8d57-3276a0305138';
...
const onDeviceFound = (error: BleError | null, device: Device | null) => {
    if (device && !devices.map(({ id }) => id).includes(device.id)) {
        ...
        onScanResult &&
            onScanResult(devices.map(({ id, name }) => ({ id, name })));
    }
};
...
const connectToDevice = (deviceId: string) => {
    ...
    bleManager?.connectToDevice(deviceId).then((d: Device) => {
        ...
        device = d;
        store.dispatch(deviceConnectedAction());
    });
};
...
const startRecipe = (recipe: Recipe) => {
    ...
    device?.discoverAllServicesAndCharacteristics().then(() => {
        const message = Buffer.from(
            JSON.stringify({ ingredients: recipe.ingredients }),
        ).toString('base64');
        device
            ?.writeCharacteristicWithResponseForService(
                SERVICE_UUID,
                RECIPE_CHARACTERISTIC_UUID,
                message,
            )
            .then(() => {
        ...
        device?.monitorCharacteristicForService(
            SERVICE_UUID,
            CURRENT_INGREDIENT_CHARACTERISTIC_UUID,
            (error, characteristic) => {
                if (error) {
                    console.log('err1', error);
                    return;
                }
                const ingredientId = new Buffer(
        ...
        device?.monitorCharacteristicForService(
            SERVICE_UUID,
            CURRENT_MASS_CHARACTERISTIC_UUID,
            (error, characteristic) => {
                if (error) {
                    console.log('err2', error);
                    return;
                }
                const mass = new Buffer(
                    characteristic?.value || '',
                    'base64',
                ).toString();
