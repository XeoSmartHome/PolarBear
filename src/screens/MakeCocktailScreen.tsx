...
const MakeCocktailScreen = () => {
    ...
    useEffect(() => {
        if (level > currentIngredient.targetLevel) {
            setCurrentIngredient(
                mappedIngredients[
                    Math.min(
                        mappedIngredients.indexOf(currentIngredient) + 1,
    ...
    return (
        <View style={styles.screen}>
            <Text style={styles.ingredient}>{currentIngredient.name}</Text>
            <Glass
                width={Dimensions.get('screen').width * 0.8}
                dimensions={{ height: 800, topWidth: 800, bottomWidth: 600 }}
                level={level}
            ...
            <Button
                title={'add'}
                onPress={() => setLevel(level => level + 0.05)}
            />
            <Button
                title={'remove'}
                onPress={() => setLevel(level => Math.max(level - 0.05, 0.001))}
            />
        </View>
    );
};
